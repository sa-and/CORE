"""Script that implements our DQN approach with a random intervention at each step"""
import random
...
class DQNrandINT(OffPolicyAlgorithm):
    """
    Deep Q-Network (DQN)
    ...
    def train(self, gradient_steps: int, batch_size: int = 2056) -> None:
        ...
        if self._n_updates % 100 == 0:
            wandb.log({"train/n_updates": self._n_updates,
                       "train/loss_cd": np.mean(losses_cd).astype('float32')}, commit=False)
    ...
    def predict(
        ...
        if not deterministic and np.random.rand() < self.exploration_rate:
            ...
            action_in = np.random.randint(0, self.policy.act_dim_in, size=action_cd.shape)
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            actions[:, 0] = np.random.randint(0, self.policy.act_dim_in, size=actions[:, 0].shape)

            # Rescale and perform action
            new_obs, rewards, dones, infos = env.step(actions)
            [wandb.log({}, commit=True) for _ in range(len(new_obs))]
